...
{
    public class FeederMaggot : ModNPC
    {
            ...
            if (Main.netMode != 1)
            {
                ...
                if (npc.ai[0] == 0)
                {
                    ...
                    for (int i = 0; i < randomWormLength; ++i)
                    {
                        ...
                        latestNPC = NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, mod.NPCType("FeederMaggotBody"), npc.whoAmI, 0, latestNPC);
                        Main.npc[(int)latestNPC].realLife = npc.whoAmI;
                        Main.npc[(int)latestNPC].ai[3] = npc.whoAmI;
                    }
                    // When we're out of that loop, we want to 'close' the worm with a tail part!
                    latestNPC = NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, mod.NPCType("FeederMaggotTail"), npc.whoAmI, 0, latestNPC);
                    Main.npc[(int)latestNPC].realLife = npc.whoAmI;
                    Main.npc[(int)latestNPC].ai[3] = npc.whoAmI;
		...
		{
			Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, mod.ItemType("HorribleClump"));
			
			if (Main.rand.Next(3) == 0)
				Item.NewItem(npc.getRect(), mod.ItemType("HorribleClump"), 2);
			
			if (Main.rand.Next(5) == 0)
				Item.NewItem(npc.getRect(), mod.ItemType("HorribleClump"), 1);
			
			if (Main.rand.Next(9) == 0)
				Item.NewItem(npc.getRect(), mod.ItemType("HorribleClumpe"), 1);
		}  
		public override float SpawnChance(NPCSpawnInfo spawnInfo)
		{
			if (SaobiesModWorld.downedTheMaw)
			{
				return SpawnCondition.Underworld.Chance * 0.000005f;
			}
