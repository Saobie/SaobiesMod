...
namespace SaobiesMod.NPCs.Worm
{
    public class MiniMaw : ModNPC
    {
        ...
        {
            npc.lifeMax = 6000;        //this is the npc health
            npc.damage = 20;    //this is the npc damage
            ...
            npc.value = Item.buyPrice(0, 0, 45, 10);
            npc.npcSlots = 1f;
            npc.netAlways = true;
        ...
        public override bool PreAI()
        {
            if (Main.netMode != 1)
            {
                ...
                if (npc.ai[0] == 0)
                {
                    ...
                    int randomWormLength = Main.rand.Next(15, 21);
                    for (int i = 0; i < randomWormLength; ++i)
                    {
                        ...
                        latestNPC = NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, mod.NPCType("MiniMawBody"), npc.whoAmI, 0, latestNPC);
                        Main.npc[(int)latestNPC].realLife = npc.whoAmI;
                        Main.npc[(int)latestNPC].ai[3] = npc.whoAmI;
                    }
                    // When we're out of that loop, we want to 'close' the worm with a tail part!
                    latestNPC = NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, mod.NPCType("MiniMawTail"), npc.whoAmI, 0, latestNPC);
                    Main.npc[(int)latestNPC].realLife = npc.whoAmI;
                    Main.npc[(int)latestNPC].ai[3] = npc.whoAmI;
		...
		public override void NPCLoot()
		{
			if (Main.expertMode)
			{
				Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ItemID.Ichor, 5);
				Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ItemID.CursedFlame, 5);
			
			if (Main.rand.Next(3) == 0)
				Item.NewItem(npc.getRect(), mod.ItemType("HorribleClump"), 4);
			
			if (Main.rand.Next(5) == 0)
				Item.NewItem(npc.getRect(), mod.ItemType("HorribleClump"), 2);
			
			if (Main.rand.Next(9) == 0)
				Item.NewItem(npc.getRect(), mod.ItemType("HorribleClumpe"), 2);
			
			if (Main.rand.Next(15) == 0)
				Item.NewItem(npc.getRect(), mod.ItemType("HorribleMaggot"), 1);
		...
		public override float SpawnChance(NPCSpawnInfo spawnInfo)
		{
			...
			{
				return SpawnCondition.Underworld.Chance * 0.0000000005f;
			}
			return 0;
		}
