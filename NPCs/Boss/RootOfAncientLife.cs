...
{
	[AutoloadBossHead]
    public class RootOfAncientLife : ModNPC
    {
        ...
        {
            npc.lifeMax = 810250;        //this is the npc health
            npc.damage = 300;    //this is the npc damage
            npc.defense = 60;         //this is the npc defense
            npc.knockBackResist = 0f;
            npc.width = 70; //this is where you put the npc sprite width.     important
            ...
            npc.value = Item.buyPrice(0, 60, 0, 10);
            npc.npcSlots = 1f;
            npc.netAlways = true;
			music = mod.GetSoundSlot(SoundType.Music, "Sounds/Music/RootOfLife/RootOfLifeMusic"); // <3
        ...
        public override bool PreAI()
        {
            if (Main.netMode != 1)
            {
                ...
                if (npc.ai[0] == 0)
                {
                    ...
                    int randomWormLength = Main.rand.Next(30, 31);
                    for (int i = 0; i < randomWormLength; ++i)
                    {
                        ...
                        latestNPC = NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, mod.NPCType("RootOfAncientLifeBody"), npc.whoAmI, 0, latestNPC);
                        Main.npc[(int)latestNPC].realLife = npc.whoAmI;
                        Main.npc[(int)latestNPC].ai[3] = npc.whoAmI;
                    }
                    // When we're out of that loop, we want to 'close' the worm with a tail part!
                    latestNPC = NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, mod.NPCType("RootOfAncientLifeTail"), npc.whoAmI, 0, latestNPC);
                    Main.npc[(int)latestNPC].realLife = npc.whoAmI;
                    Main.npc[(int)latestNPC].ai[3] = npc.whoAmI;
			...
			if(projectileTimer1++ == 180) // EVERY 60 TICKS EQUALS TO 1 SECOND SO THIS CAUSES BOSS TO SHOOT EVERY 3 SECOND
			{
				...
				int damage = 85;  // The damage your projectile deals.
				float knockBack = 5;
				int type = ProjectileID.DD2OgreSpit; // The type of the projectile you want to shoot.
				...
				if(projectileTimer2++ == 180) 
			{
				...
				float projectileSpeed = 14f;  // The speed of your projectile (in pixels per second).
				int damage = 65;  // The damage your projectile deals.
				float knockBack = 5;
		...
		public override void NPCLoot()
		{
			...
			Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, mod.ItemType("AncientLifewood"), Main.rand.Next(25, 100));
			Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, mod.ItemType("AncientLifeShard"), Main.rand.Next(1, 2));
			Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, mod.ItemType("SummonerStone"), Main.rand.Next(200, 300));
			Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ItemID.LifeCrystal, Main.rand.Next(1, 3));
			Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, ItemID.LifeFruit, Main.rand.Next(2, 5));
			
			
			
			if (Main.rand.Next(3) == 0)
				Item.NewItem(npc.getRect(), mod.ItemType("AncientLifewood"), 25);
			
			if (Main.rand.Next(9) == 0)
				Item.NewItem(npc.getRect(), ItemID.LifeFruit, 1);
			...
			if (Main.rand.Next(9) == 0)
				Item.NewItem(npc.getRect(), ItemID.LifeCrystal, 1);
			...
			if (Main.rand.Next(100) < 5)
				Item.NewItem(npc.getRect(), mod.ItemType("EssenceOfAncientLife"), 1);
			
			if (Main.rand.Next(20) < 5)
				Item.NewItem(npc.getRect(), mod.ItemType("AncientLifeShard"), 1);
